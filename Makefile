...
KDIR = linux-rt
KVER = $(shell make -s show_version)
...
show_version: kernel-config
	$(MAKE_ARCH) -C $(KDIR) kernelrelease
...
modules-prepare: kernel-config
	$(MAKE_ARCH) -C $(KDIR) modules_prepare

kernel-modules: kernel-config modules-prepare
	mkdir -p $(STAGE_DIR)
	rm -rf $(STAGE_DIR)/lib
	$(MAKE_ARCH) -C $(KDIR) -j$(J) modules
	$(MAKE_ARCH) -C $(KDIR) INSTALL_MOD_STRIP=1 INSTALL_MOD_PATH=$(STAGE_DIR) modules_install
...
drivers-install: drivers
	...
	cp -raf phoenix/system/src/bin/* $(STAGE_DIR)/lib/modules/$(KVER)/kernel/extra
...
modules:kernel-modules drivers-install
	$(MAKE_ARCH) -C $(KDIR) INSTALL_MOD_PATH=$(STAGE_DIR) _depmod
